...
class Studiengang:
    ...
    def kurs_auslesen(self, mid):
        """ 
        Für das Auslesen einzelner Kurse mit ihrer ID "mid".
        Gibt None zurück, wenn Kurs nicht existiert
        """
    ...
    def metrics_mean(self):
        ...
        Gibt None zurück, wenn keine gültigen Noten vorhanden sind.
        """             
        noten = []
        for m in self.module:
    ...
    def metrics_ects(self):
        """
        Berechnet das Paar bestandene ECTS und nötige ects_max 
        """         
        ects_max = 0
        ects = 0
...
class Kurs:
    ...
    def kursstatus_belegt(self):
        """Gibt True zurück, wenn ein Kurs belegt ist """    # Wird von Dashboard.build_dashboard_datapackage() genutzt
        return self.belegt

    def kursstatus_bestanden(self):
        """
        True wenn ein Kurs bestanden ist - Wenn die Note <= 4 ist.
        Auch 0 zählt als bestanden (Anerkennung) 
        """   
...
class API:
    """
    Flask-API Routing Endpunkte:
     GET   /                      -> rendert index.html
     GET   /api/module/<mid>      -> liefert konkrete Moduldaten anhand der ID (mid)
     POST  /api/module/<mid>      -> aktualisiert Moduldaten anhand der ID (mid)
    """
...
class Dashboard:
    ...
    def module_get(self, mid):
        """
        Gibt Modul über die ID (mid) zurück
        """
...
class Eingabe:
    """   
    Einfache Validierungschecks für Titel und Note durch Nutzereingabe
    StaticMethod als wichtiger Aspekt. Muss nicht über Instanz aufgerufen werden.
    """
    @staticmethod
...
class Speicher:
    """
    Speichert den Studiengang (Objekt) wieder als JSON-Struktur
    Beim Lesen umgekehrt (baut Objekt aus JSON-Struktur)
    Wenn noch keine data.json existiert, wird sie jetzt erzeugt
    """ 
...
class KursFactory:
    """
    Erzeugt einen Standard-Datensatz beim ersten Start
    """
...
def create_app():
    """ 
    Baut die Schichten zur Flask-App zusammen
    """
